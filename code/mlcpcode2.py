...
###
#be sure to change [YOUR BASE DIRECTORY] to the path for your working directory
###
...
def cifgenbase(geo,sites,positions,name):
    geometry={}
    geometry=Lattice.from_dict({'a':geo[0],
                                  'b':geo[1],
                                  'c':geo[2],
                                  'alpha':geo[3],
                                  'beta':geo[4],
                                  'gamma':geo[5]})
    struct=Structure(geometry, sites, positions)
    structcif=struct.to(filename=f"{name}.cif")
    return f"{name}.cif"

def cifgen(dataset,name):
    geometry={}
    geometry=Lattice.from_dict({'a':dataset['real_geometry'][0],
                                  'b':dataset['real_geometry'][1],
                                  'c':dataset['real_geometry'][2],
                                  'alpha':dataset['real_geometry'][3],
                                  'beta':dataset['real_geometry'][4],
                                  'gamma':dataset['real_geometry'][5]})
    struct=Structure(geometry, dataset['fixedrealsites'], dataset['positions'])
    structcif=struct.to(filename=f"{name}.cif")
    return f"{name}.cif"

def cifgenrelax(dataset,name):
    geometry={}
    geometry=Lattice.from_dict({'a':dataset['geometry'][0],
                                  'b':dataset['geometry'][1],
                                  'c':dataset['geometry'][2],
                                  'alpha':dataset['geometry'][3],
                                  'beta':dataset['geometry'][4],
                                  'gamma':dataset['geometry'][5]})
    struct=Structure(geometry, dataset['fixedsites'], dataset['positions'])
    structcif=struct.to(filename=f"{name}.cif")
    return f"{name}.cif"
...
def mlcp(cif,sf=1.0): 
            ...
            for j in range(len(neighbors[i])): #for each neighbor atom of each one
                ...
                if [i, k, sc, nc] in unique or [k, i, nc, sc] in unique: #so as to not repeat a contact twice from each's perspective
                    ...
                    for l in range(2):
                        ...
                        cns_data.append(abs(cns[i][l]-cns_neighbor[l])/2) #same thing? should be neg??? (changed) oh no !!!
        ...
        if i in filter_indicies: # for each atom
            for j in range(len(predictions_rfr)): #each contact
                if i in used_for[j]: #if the atom we are looking at in is the contact
                    ...
                    scale = weights*neighbor_weights[j]*predictions_rfr[j]/len(vg) #getting a micro prediction for each conteact, basically, adds up to prediction
    ...
    os.chdir('[YOUR BASE DIRECTORY]'+directory_name)
